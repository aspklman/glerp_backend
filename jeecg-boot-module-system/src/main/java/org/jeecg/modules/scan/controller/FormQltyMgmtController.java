...
public class FormQltyMgmtController {
    ...
    public Result<IPage<FormQltyMgmt>> queryPageList(FormQltyMgmt formQltyMgmt,
        ...
        String proFact = formQltyMgmt.getProFact();
        queryWrapper.and(wrapper -> wrapper.eq("pro_fact", proFact));
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-不良原因名称及数量查询(中国)")
    @ApiOperation(value = "成型品质管理-不良原因名称及数量查询(中国)", notes = "成型品质管理-不良原因名称及数量查询(中国)")
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-不良原因名称及数量查询(越南)")
    @ApiOperation(value = "成型品质管理-不良原因名称及数量查询(越南)", notes = "成型品质管理-不良原因名称及数量查询(越南)")
    @GetMapping(value = "/getBadNoQtyVn")
    public Result<List<String>> getBadNoQtyVn(@RequestParam("pssr") String pssr) {
        Result<List<String>> result = new Result<List<String>>();
        if (pssr == null) {
            result.error500("参数不识别！");
        } else {
            ...
            log.info("日期：" + recDate);
            log.info("====================");
            List<String> stgscanQualityList = this.stgscanQualityService.getBadNoQtyVn(proFact, styleNo, secNo, recDate);
            result.setResult(stgscanQualityList);
            result.success("获取成功!");
        }
        log.info("结果：" + result);
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-插入(中国)")
    @ApiOperation(value = "成型品质管理-插入(中国)", notes = "成型品质管理-插入(中国)")
...
//             log.info("时间：" + recTime);
//             log.info("用户：" + userNo);
            this.stgscanQualityService.insertBadNoQty(factNo, proFact, styleNo, secNo, recDate, badLevel, badNo, addValue, recTime, userNo);
            result.success("获取成功!");
        }
//         log.info("结果：" + result);
//         return result;
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-插入(越南)")
    @ApiOperation(value = "成型品质管理-插入(越南)", notes = "成型品质管理-插入(越南)")
    @GetMapping(value = "/insertBadNoQtyVn")
    public void insertBadNoQtyVn(@RequestParam("pssrbba") String pssrbba) {
        Result result = new Result();
        if (pssrbba == null) {
            result.error500("参数不识别！");
        } else {
...
//             log.info("时间：" + recTime);
//             log.info("用户：" + userNo);
            this.stgscanQualityService.insertBadNoQtyVn(factNo, proFact, styleNo, secNo, recDate, badLevel, badNo, addValue, recTime, userNo);
            result.success("获取成功!");
        }
//         log.info("结果：" + result);
//         return result;
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-删除(中国)")
    @ApiOperation(value = "成型品质管理-删除(中国)", notes = "成型品质管理-删除(中国)")
...
//			 log.info("不良原因：|" + badNo + '|');
//			 log.info("用户：|" + userNo + '|');
            this.stgscanQualityService.deleteBadNoQty(factNo, proFact, styleNo, secNo, recDate, badNo, userNo);
            result.success("获取成功!");
        }
//		 log.info("结果：" + result);
//		 return result;
     ...
     * @return
     */
    @AutoLog(value = "成型品质管理-删除(越南)")
    @ApiOperation(value = "成型品质管理-删除(越南)", notes = "成型品质管理-删除(越南)")
    @GetMapping(value = "/deleteBadNoQtyVn")
    public void deleteBadNoQtyVn(@RequestParam("pssrb") String pssrb) {
        Result result = new Result();
        if (pssrb == null) {
            result.error500("参数不识别！");
        } else {
...
//			 log.info("不良原因：|" + badNo + '|');
//			 log.info("用户：|" + userNo + '|');
            this.stgscanQualityService.deleteBadNoQtyVn(factNo, proFact, styleNo, secNo, recDate, badNo, userNo);
            result.success("获取成功!");
        }
//		 log.info("结果：" + result);
//		 return result;
